...
def updateUserLocation():
    ...
    if user is None:
        ...
        user.point = geoalchemy2.WKTElement(
            "POINT({} {})".format(longt, lat), srid=4326
        )
        db.session.commit()
        statuscode = 200
...
def updateUserInfo():
    ...
    if user is None:
        ...
        possibleUserWithEmail = (
            User.query.filter(User.id != userID).filter(User.email == newEmail).first()
        )
        possibleUserWithUsername = (
            User.query.filter(User.id != userID)
...
def getAllUsers():
    ...
    res = {"data": [user.serialize() for user in allUsers]}
